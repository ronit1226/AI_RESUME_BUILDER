                    ...
                    {data.map((experience, index)=>(
                        <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                            ...
                            <div className='grid md:grid-cols-2 gap-3'>
                                ...
                                <label>
                                    <input type="checkbox" checked={experience.is_current || false} onChange={(e)=>{updateExperience(index,"is_current", e.target.checked ? true : false);}}className='rounded border-blue-300 text-blue-600 focus:ring-blue-500' />
                                    <span className='text-sm text-gray-700 ml-2'>Currently working here</span>
                                </label>
                                <div className='space-y-2 mt-2'>
                                    <div className='flex items-center justify-between'>
                                        <label className='flex items-center gap-1 px-2 py-1 text-xs  rounded transition-colors diabled:opacity-50'>Job Description</label>
